            ...
            success: function(msg) {
                ...
                if (returnedArray != null) {
                    for (i = 0; i < returnedArray.length; i++) {
                        ...
                        for (s = 1; s < 15; s++) {
                            if (s == 13 && $("#cartype").val() == 1) {
                                $(idprice + s).append("<option value='" + returnedArray[i].price2 + "'>" + returnedArray[i].name + "</option>");
                            } else {
...
async function runtitle(id) {
    $("#title").empty();
    $("#title").append('<option value="0" selected  disabled >กรุณาเลือก</option>');
    if (id == "" || id == undefined) id = "1";
    const carTypeList = await loadTitle(id);
    const carTypeElem = document.querySelector('#title');
...
async function runCustomer() {
    const CusList = await loadCustomer();

    const CusListElem = document.querySelector('#Dxuser');
    if (CusList.Status == 200) {
        CusList.Data.forEach(x => {
            const op = document.createElement('option');
            op.value = x.Id;
            op.text = `[${x.Id}] ${x.Name}`;
            CusListElem.appendChild(op);
        });
    }
}
...
async function runProvince() {
    ...
    $("#car_regis_pro").append('<option value="0" selected  disabled >กรุณาเลือก</option>');
    $("#province").append('<option value="0" selected  disabled >กรุณาเลือก</option>');
    const carProvinceElem = document.querySelector('#car_regis_pro');
    const carProvince = document.querySelector('#province');
    ...
    if (carTypeList.Status == 200) {
        ...
        carTypeList.Data.forEach(x => {
            ...
            op.value = x.Id;
            op.text = x.Name;
            carProvinceElem.appendChild(op);
        });
    }
}
