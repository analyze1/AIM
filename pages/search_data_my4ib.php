...
<a onclick ="reportexcel()" class="btn">Export Excel</a>
...
// end สลักหลังเลขตัวถัง
  ...
  ?>
      ...
      ?>
      <? if($row['Edit_CarBody'] != '' ){ $rescarbody = $row['Edit_CarBody']; }else{  $rescarbody = $row['car_body']; } ?>
    <tr align="center">
      <td style='vertical-align:top;'><input type="checkbox" id="chkid<?=$n;?>" name="chkid[]" value="<?=$row['id_data'].'|'.$insureename.'|'.$dataaddr.'|'.$dataaddr1.'|'.$dataaddr2.'|'.DateThai($row['send_date']).'|'.$mo_sub_name.'|'.$rescarbody;?>"></td>
      <td align="center" valign="top"  ><?=DateThai($row['send_date'])?></td>
      <td height="36" align="center" valign="top"><?=$row['id_data'];?></br><? echo '<font color="#FF0000">'.$row['n_insure'].'</font>';?></td>
...
<input style="display:none;" type="submit" id="btnfrmPrint"  name="btnfrmPrint">
</form>
</div>
<script>
 ...
 function  reportexcel(){
    $('#frmPrint').attr('action', 'report/report_insure_data_xls.php');
    $('#frmPrint').submit();
 }
