...
$objPHPExcel->getActiveSheet()->getStyle($cell_fix.$row.':'.$cell.$row)->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->getStyle($cell_fix.$row.':'.$cell.$row)->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_DOUBLE);
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, 'รวม');
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, $sum_unit);
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, number_format($sum_car_price,2,'.',','));
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, number_format($sum_car_cost,2,'.',','));
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, number_format($sum_car_bsc14,2,'.',','));
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, number_format($sum_car_bsc16,2,'.',','));
$cell_fix++;
$objPHPExcel->getActiveSheet()->SetCellValue($cell_fix.$row, number_format($sum_car_bsc2,2,'.',','));

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
$objPHPExcel->getActiveSheet()->setTitle('รายการสั่งซื้อ'.str_replace('/','-',$q_auto));
...
header('Content-Disposition: attachment;filename="รายการสั่งซื้อ'.str_replace('/','-',$q_auto).'.xls"');
