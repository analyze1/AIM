...
foreach ($objQuery as $row) {
	...
	if ($changeCode <= 201704) {
		...
		if ($arrRenew['quotationIdFour'] != '') {
			$sql = "SELECT * FROM quotation WHERE q_auto = '$arrRenew[quotationIdFour]'";

           
			$qry = $_contextFour->query($sql)->fetch(2);
			$pa1 = $qry['human_amt'];
			$pa2 = $qry['asset_amt'];
			$pa3 = $qry['drive1_amt'];
			$pa4 = $qry['passenger_amt'];
			$pa5 = $qry['medic_amt'];
			$pa6 = $qry['criminal_amt'];
			$seat = $qry['passenger'];
			$row['car_regis'] = $qry['id_car_regis'];
		} else {
	...
	$pdf->AddPage();
	$pdf->Image('./images/payment_company_quotation.jpg', 0, 0, 210);
	...
	if(!empty($checkRenewRe))//TODO เพิ่ม approve telephone
	{
		...
		if($arrRenew['quotationIdFour'] != ''){
			$cashBefore = "สอบถามข้อมูลเพิ่มเติม $checkRenewRe->nameFull";
		}else{
	...
	if ($arrRenew['end_date'] == '0000-00-00' || $arrRenew['end_date'] == '') {
		...
		$end_date = $arrRenew['end_date'];
	}
	$pdf->SetFont('angsa', '', 14);
	$pdf->SetX(44);
	$pdf->Cell(5, 8, iconv('UTF-8', 'TIS-620', ''), 0, 0, 'L');
	$pdf->SetFont('angsa', 'B', 14);
	$pdf->SetX(62);
	$pdf->Cell(40, 8, iconv('UTF-8', 'TIS-620', thaiDate(date('Y-m-d', strtotime($end_date . ' -1 YEAR'))) . '-' . thaiDate($end_date)), 0, 0, 'C');
	...
	if($arrRenew['quotationIdFour']){
		$net_renew = $exRe[8];
		$totalAll = number_format($exRe[8],2);
	}else{
		$totalAll = number_format(($total_pre + $act_cost - $sumextra) - $arrvat_total, 2);
		$net_renew = ($total_pre + $act_cost - $sumextra) - $arrvat_total;
	}

	$pdf->SetY(136.5); //129
	$pdf->SetX(62);
	$pdf->Cell(20, 6, iconv('UTF-8', 'TIS-620', $totalAll), 0, 0, 'R'); //รวมพรบ
	...
	$excarbody = 'MV' . $exid_data[2]; //ใช้เลขรับแจ้ง 6 หลักสุดท้ายแทนเป็น ref 2 
	$exid_data[0] = '829';
	$exid_data[2] = '113100';
	...
	$pdf->Cell(0, 0, iconv('UTF-8', 'TIS-620', '08' . $exid_data[0] . $exid_data[2]), 0);
	...
	$pdf->SetY(246); //263 จำนวนเงินตัวอักษร
	$pdf->SetX(39.5);
	//$pdf->Cell(0,0,iconv( 'UTF-8','TIS-620',$row['car_regis']." ".$c_regis),0);
	...
	$codeN = "|010755500013900 08" . $exid_data[0] . '' . $exid_data[2] . " " . $excarbody . " " . $numcutdot . "";

	$code = "|010755500013900\n08" . $exid_data[0] . '' . $exid_data[2] . "\n" . $excarbody . "\n" . $numcutdot . "";
