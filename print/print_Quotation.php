...
while($row = mysql_fetch_array($objQuery))
{
	...
	*/
	...
	$dateYear=substr($row['end_date'],0,4);
	$dateMonth=substr($row['end_date'],5,2);
	$dateY=date('Y');
	...
	*/
	...
	mysql_select_db($db2,$cndb2);
	$tb_cost_sql="SELECT namegroup,comp_sort FROM tb_cost_mocar WHERE  cmocar_sz IN ('".$row['mo_car']."','ALL') AND cstatus_sz = 'Y' AND
	YEAR(cm_datestart) <=  '".$dateYear."' AND YEAR(cm_expire) >= '".$dateYear."' AND
	MONTH(cm_datestart) <=  '".$dateMonth."' AND MONTH(cm_expire) >= '".$dateMonth."' AND
	ins_type = '1' order by comp_sort='VIB_S103' DESC, comp_sort='VIB_S' DESC limit 1";

//echo $tb_cost_sql.'<br>';
$tb_cost_query=mysql_query($tb_cost_sql,$cndb2);
$tb_cost_array=mysql_fetch_array($tb_cost_query);
$tb_num_cost = mysql_num_rows($tb_cost_query);
if($tb_num_cost>0){
	$select_renew_sql="SELECT net FROM tb_cost WHERE cost <= '".$ResultCost."' AND cost_end >= '".$ResultCost."' AND used_suzuki IN ('A','R') AND comp = '".$tb_cost_array['comp_sort']."' AND car_old <= '".$carold."' AND car_old_end >= '".$carold."' AND car_id = '".$row['car_id']."'
	AND YEAR(create_date)  <= '".$dateYear."'AND YEAR(date_expired) >= '".$dateYear."'
	AND MONTH(create_date)  <= '".$dateMonth."'AND MONTH(date_expired) >= '".$dateMonth."'
	AND mocargroup = '".$tb_cost_array['namegroup']."'";
//echo $select_renew_sql.'<br>';
	$select_renew_query=mysql_query($select_renew_sql,$cndb2);
	$select_renew_array=mysql_fetch_array($select_renew_query);
}
	if(!empty($select_renew_array))
	{
		$total_pre=$select_renew_array['net'];
	}
	else
	{
		$total_pre=0;
