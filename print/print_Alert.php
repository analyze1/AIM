...
$query = "SELECT *,URenew.renew_id AS IdS FROM data ";
...
if($objQuery)
{
    ...
    $mail->Password = "12345"; // รหัสผ่าน SMTP
	 $mail->Subject = "มีการพิมพ์ใบเตือนต่ออายุ ".$_SESSION["strUser"]; // กำหนดหัวข้ออีเมล์
    $mail->Body = $_SESSION["strUser"]."  ได้พิมพ์ใบเตือน $IDDATA";
...
$query_detailRenew = "SELECT * FROM detail_renew WHERE  detail_renew.id_data ='".$IDDATA."' order by id_detail desc limit 1";
...
$new_detail = explode("|",$row_detailRenew['detailcost']);
...
if($row['car_id']==110)
{
	if($row_detailRenew['detail_doc_type'] == '' || $row_detailRenew['detail_doc_type'] == '1')
	{
		$act_cost = '645.21';
		$pa1='1,000,000';
		$pa2='5,000,000';
	...
	else if($row_detailRenew['detail_doc_type'] == '2+')
	{
		$act_cost = '645.21';
		$pa1='300,000';
		$pa2='1,000,000';
...
{
	if($row_detailRenew['detailcost'] == '' || $row_detailRenew['detailcost'] == '1')
	{
		$act_cost = '967.28';
		$pa1='300,000';
		$pa2='1,000,000';
	...
	else if($row_detailRenew['detailcost'] == '2+')
	{
		$act_cost = '645.21';
		$pa1='300,000';
		$pa2='1,000,000';
...
else
{
	...
	$pdf->SetX(8);
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620','บริษัทขอสงวนสิทธิ์ ในการเสนออัตราเบี้ยประกันภัยและคุ้มครองดังกล่าว จนถึงวันที่ '.thaiDate2($row_detailRenew['date_detail']).' (นับจากวันเสนอ 30 วัน)'),0,0,'L');
	$pdf->SetY(97);
	$pdf->SetX(8);
	...
	$pdf->SetY(131);
	$pdf->SetX(75);
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620',number_format($row['renew_net'],2)),0,0,'L');
	$pdf->SetY(140);
	$pdf->SetX(75);
	...
	$pdf->SetY(149);
	$pdf->SetX(75);
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620',number_format($row['renew_net']+$act_cost-$row['renew_discountExtra']-$row['renew_discount_amt'],2)),0,0,'L');
	$pdf->SetY(116);
	$pdf->SetX(70);
