...
for($xx=0;$xx<count($addonarray);$xx++)
{
	if($addonarray[$xx] == "ADDON_PV")
			{
				...
				$sqladdon_query =mysql_query($sqladdon_sql);
				$sqladdon_array=mysql_fetch_array($sqladdon_query);
	...
	else if($row['car_id'] == "220" OR $row['car_id'] == "230")
			{
				$frist = "-";
				$frist_1_1 = "";
			}
	...
	$cost = str_replace(",","",substr($row['cost'],0,7));
...
if($ShowReq =='' && $ShowReqOld =='')
{
	...
	{	
		...
		else if($row['car_cat_acc'] == '120')
		{
			if($row['EditProduct'] =='Y')
			{
				$type_acc = 'สลักหลังเปลี่ยนประเภทรถ : (120  รถยน์นั่งใช้เพื่อการพาณิชย์ ไม่ใช่รับจ้างสาธารณะ) เพิ่มทุน'.number_format($row['TotalProduct']).' บาท เพิ่มเบี้ย '.number_format($row['car_cat_acc_total'],2).' บาท';
			}
			else
			{
				$type_acc = 'สลักหลังเปลี่ยนประเภทรถ : (120  รถยน์นั่งใช้เพื่อการพาณิชย์ ไม่ใช่รับจ้างสาธารณะ) เพิ่มทุน'.number_format($row['price_total']).' บาท เพิ่มเบี้ย '.number_format($row['car_cat_acc_total'],2).' บาท';
			}
		}
		$pdf->SetY(224);
...
else
{
	$SendAdd = ' (กรุณาจัดส่งเอกสารมาที่ : '.$row['SendAdd'].')';
		...
		$pdf->SetFont('angsa','B',12);
		$pdf->SetX(100);
		$pdf->Cell(60,6,iconv('UTF-8','TIS-620','บริษัท โฟร์ อินชัวรันส์ โบรกเกอร์ จำกัด'),0,0,"L");
		$pdf->SetFont('angsa','',12);
		$pdf->SetX(120);
...
else if($ShowReq != '' || $ShowReqOld != '')
{
	...
	else
	{
		...
		if($TotalPrice ==0.00)
		{
			//$pdf->Cell(0,16,iconv('UTF-8','TIS-620','เพิ่มทุนอุปกรณ์รวม '.$TotalProduct.' บาท'.'เพิ่มเบี้ยรวม '.$TotalPrice."  บาท"." (วันที่สลักหลัง : ".date('d/m/Y',strtotime($row['Req_Date'])).") "),0,0,"R");
		}
		else
		{
			$pdf->Cell(0,16,iconv('UTF-8','TIS-620','เพิ่มทุนอุปกรณ์รวม '.$TotalProduct.' บาท '.'เพิ่มเบี้ยรวม '.$TotalPrice."  บาท"." (วันที่สลักหลัง : ".date('d/m/Y',strtotime($row['Req_Date'])).") "),0,0,"R");
