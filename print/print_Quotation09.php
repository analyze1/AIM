...
require('../fpdf.php');
	...
	$row_user = mysql_fetch_array( $objQuery_user );
        ...
        mysql_select_db($db1,$cndb1);
        $objQuerycontact = mysql_query($querycontact);
        $row_userHR = mysql_fetch_array($objQuerycontact);
...
list($det_date,$detail_time) = split(' ',$row_Drenew['date_detail']); // แยกวันที่ กับ เวลาออกจากกัน
	...
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620',$address_pdf3.' '.$row['postal']),0,0,'L');
        ...
        if(!empty($row['SendAdd'])){
        $pdf->SetFont('angsa','',14);
        $pdf->SetTextColor(255,0,0);
        $pdf->SetY(68);
	$pdf->SetX(5);
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620','* ที่อยู่ในการจัดส่ง'),0,1,'L');
        
        $pdf->SetTextColor(0,0,0);
        $pdf->SetY(74);
	$pdf->SetX(7);
	$pdf->Cell(0,5,iconv('UTF-8','TIS-620',$row['SendAdd']),0,1,'L');
        }
	...
	$pdf->SetX(120);
	$pdf->SetFont('angsa','',14);
	$pdf->Cell(66,6,iconv('UTF-8','TIS-620','แจ้งชำระเบี้ยประกันภัยและสอบถาม คุณ '.$row_userHR['emp_namerenew']),0,1,'C');
	
	$pdf->SetX(120);
	$pdf->Cell(66,5,iconv('UTF-8','TIS-620','โทร. : 021968234 แฟกซ์. : 021968235 Email : '.$row_user['emp_emailrenew']),0,1,'C');
	...
	if($row['add_on1']=='1')
	{
            $pdf->Image('../images/sukjai1.jpg',107,140,98,20);
            //$chip=430;
	}
	else if($row['add_on1']=='2')
	{
            $pdf->Image('../images/sukjai2.jpg',107,140,98,20);
           // $chip=430;
        }else{
	...
	$pdf->SetY(129);
	$pdf->SetX(73);
	$pdf->Cell(17,5,iconv('UTF-8','TIS-620',number_format($new_detailcost[8],2)),0,0,'R');  //ยอดลูกค้าชำระ
	...
	$pdf->SetY(95.5);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa1,0)),0,0,'R');
	
	$pdf->SetY(100.5);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa2,0)),0,0,'R');
	
	$pdf->SetY(115.5);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa3,0)),0,0,'R');
	
	$pdf->SetY(121);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa4,0)),0,0,'R');
	...
	$pdf->SetY(126);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa5,0)),0,0,'R');
	
	$pdf->SetY(131);
	$pdf->SetX(165);
	$pdf->Cell(20,5,iconv('UTF-8','TIS-620',number_format($pa6,0)),0,0,'R');
