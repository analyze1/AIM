...
foreach ($objQuery as $row) {
	...
	if ($row['status_SendAdd'] == 'Y') {
		...
		if ($textaddarray[5] != "102") {
			$address_pdf2 .= 'ต.' . $_SESSION["Tum"][$textaddarray[7]] . ' อ.' . $_SESSION["Amp"][$textaddarray[6]];
			$address_pdf3 .= 'จ.' . $_SESSION["Pro"][$textaddarray[5]] . ' ' . $textaddarray[8];
		} else {
			$address_pdf2 .= 'แขวง' . $_SESSION["Tum"][$textaddarray[7]] . ' เขต' . $_SESSION["Amp"][$textaddarray[6]];
			$address_pdf3 .= $_SESSION["Pro"][$textaddarray[5]] . ' ' . $textaddarray[8];
		}
		...
		if ($row['Cus_province'] != "102") {
			$address_pdf2 .= 'ต.' . $_SESSION["Tum"][$row['Cus_tumbon']] . ' อ.' . $_SESSION["Amp"][$row['Cus_amphur']];
			$address_pdf3 .= 'จ.' . $_SESSION["Pro"][$row['Cus_province']] . ' ' . $row['Cus_postal'];
		} else {
			$address_pdf2 .= 'แขวง' . $_SESSION["Tum"][$row['Cus_tumbon']] . ' เขต' . $_SESSION["Amp"][$row['Cus_amphur']];
			$address_pdf3 .= $_SESSION["Pro"][$row['Cus_province']] . ' ' . $row['Cus_postal'];
		}
	} else {
		if ($row['add'] != "-" && $row['add'] != "") {
			$address_pdf .= $row['add'];
		...
		if ($row['province'] != "102") {
			...
			$address_pdf3 .= 'จ.' . $_SESSION["Pro"][$row['province']] . ' ' . $row['postal'];
		} else {
			$address_pdf2 .= 'แขวง' . $_SESSION["Tum"][$row['tumbon']] . ' เขต' . $_SESSION["Amp"][$row['amphur']];
			$address_pdf3 .= $_SESSION["Pro"][$row['province']] . ' ' . $row['postal'];
		}
	}
	...
	if ($changeCode <= 201704) {
		...
		//$insured_type = $arrcost['insured_type'] .'  '.$arrcost['prod_name'].'  '.$arrcost['prod_remark'] ;
	...
	$pdf->Image('../images/confirmation_form_to_policy.png', 0, 1, 210);
	// if (!empty($arrcostcomp)) {
	// 	$pdf->Image('../pic_insured/renew/' . $arrcostcomp . '.png', 160, 0, 50);
	// } else {
	// 	$pdf->Image('../pic_insured/renew/VIB_S.png', 160, 0, 50);
	// }
	...
	if ($address_pdf4 != '') {
		...
		$pdf->SetY(53);
		$pdf->SetX(20);
		$pdf->Cell(0, 5, iconv('UTF-8', 'TIS-620', $address_pdf2.'55'), 0, 0, 'L');
	...
	$pdf->Cell(40, 8, iconv('UTF-8', 'TIS-620', thaiDate($end_date) . '-' . thaiDate(date('Y-m-d', strtotime($end_date . ' +1 YEAR')))), 0, 0, 'C');
	...
	$pdf->SetY(105); //100
	$pdf->SetX(33);
	$pdf->Cell(0, 5, iconv('UTF-8', 'TIS-620', $arrCompany[$arrcostcomp]), 0, 0, 'L');
	...
	$pdf->SetY(126); //120.5
	$pdf->SetX(62);
	$pdf->Cell(20, 6.5, iconv('UTF-8', 'TIS-620', number_format($act_cost, 2)), 0, 0, 'R'); //พรบ

	$pdf->SetTextColor(255, 0, 0);

	$pdf->SetY(133); //129
	$pdf->SetX(62);
	$pdf->Cell(20, 6, iconv('UTF-8', 'TIS-620', number_format($arrvat_total, 2)), 0, 0, 'R');
	//"หัก ณ ที่จ่าย ".$arrvat_pointer."% = ".
	$pdf->SetTextColor(0, 0, 0);

	$pdf->SetY(139.5); //129
	$pdf->SetX(62);
	$pdf->Cell(20, 6, iconv('UTF-8', 'TIS-620', number_format(($total_pre + $act_cost - $sumextra) - $arrvat_total, 2)), 0, 0, 'R'); //รวมพรบ
	...
	$pdf->SetX(167);
	//$pdf->Cell(0,0,iconv( 'UTF-8','TIS-620',$row['id_data_company']),0);
	...
	$pdf->SetX(129.5);
	$pdf->Cell(40, 6, iconv('UTF-8', 'TIS-620', number_format($net_renew, 2)), 0, 0, 'C');

	$pdf->SetY(249); //263 จำนวนเงินตัวอักษร
	$pdf->SetX(39.5);
	//$pdf->Cell(0,0,iconv( 'UTF-8','TIS-620',$row['car_regis']." ".$c_regis),0);
	$num2dg = str_replace(',', '', number_format($net_renew, 2));
	...
	$idqrcode = '../qrcode/'.$qrcodename.'.png';//
	QRcode::png($code, $idqrcode, QR_ECLEVEL_L, 1);
	$pdf->Image($idqrcode,118,265,18,18);//275	*/
}
$pdf->Output();
unlink($idqrcode);