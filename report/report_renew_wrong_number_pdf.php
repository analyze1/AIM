...
$ar_width_cell = array(9, 20, 30, 20, 45, 92, 23, 23, 30);
...
foreach ($dataAll as $row) {
    ...
    if ($ch == 1) {
        ...
        $x++;
        $pdf->Cell($ar_width_cell[$x], $ar_height_cell[$x], iconv('UTF-8', 'TIS-620', 'ดีลเลอร์'), $border, 0, "C", "T");
        $x++;
        ...
        $x++;
        $pdf->Cell($ar_width_cell[$x], $ar_height_cell[$x], iconv('UTF-8', 'TIS-620', 'การครั้งโทรล่าสุด'), $border, 1, "C", "T");
        $pdf->SetFont('angsa', $ar_font_type[0], $ar_font_size[0]);
    }
    $x = 0;
    $pdf->SetX($setX[0]);
    ...
    $x++;
    $pdf->Cell($ar_width_cell[$x], $ar_height_cell[$x], iconv('UTF-8', 'TIS-620', $_informDealer[$row['login']]['nameFull']), $border, 0, "L");
    $x++;
    ...
    $pdf->Cell($ar_width_cell[$x], $ar_height_cell[$x], iconv('UTF-8', 'TIS-620', $lastCall), $border, 1, "C");
