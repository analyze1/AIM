...
$sheetname = "รายงานการแจ้งประกันภัย";
 // ชื่อ คอลัมภ์
$arrHead = array('ที่','เลขรับแจ้ง','วันที่แจ้ง','ชื่อผู้เอาประกัน','รุ่น','เลขตัวถัง');
$arrHeadwidth = array('12','15','15','30','30','30');
...
//     $txtsearch = $data1[0];
//     $search = " AND data.id_data  like '%".$txtsearch."%' ";

// //$arrHead = array('เลขรับแจ้ง','วันที่แจ้ง','ชื่อผู้เอาประกัน','วันคุ้มครอง','ยี่ห้อ','รุ่น','เลขตัวถัง');
// $query = "SELECT ";
// $query .= " detail.car_detail,data.id_data,detail.id_data,data.com_data,data.n_insure,detail.br_car,detail.mo_car,detail.add_price,detail.cat_car,data.login,detail.car_regis,data.send_date,data.start_date,data.end_date,detail.car_body,detail.n_motor,data.costCost,detail.car_id,data.p_act, req.CostProduct, req.Cus_title, req.Cus_name as reqcusname, req.Cus_last, req.EditAct_id, req.Edit_CarBody, req.CostProduct, req.Req_Status,detail.mo_sub,detail.car_cat_acc_total,detail.car_cat_acc,detail.equit,req.EditProduct,tb_br_car.name as brname ,tb_mo_car.name as moname ";
// // $query .= ",insuree.title,insuree.last,insuree.id_data,insuree.name As cus_name,insuree.add,insuree.group,insuree.town,insuree.lane,insuree.road,insuree.tumbon,insuree.amphur,insuree.province,insuree.postal ";
...
// // $query .= " tb_customer.saka,tb_tumbon.name as tumbon_name, "; 
