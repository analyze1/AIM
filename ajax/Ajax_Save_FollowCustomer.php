...
require("../email/PHPMailer/PHPMailerAutoload.php");  // เรียกไฟล์ class.phpmailer.php
	...
	$objQuery_fol= mysql_query($query_fol,$cndb2) or die ("Error _fol [".$query_fol."]");
	...
	mysql_select_db($db2,$cndb2);
	$objQuery_comp= mysql_query($query_comp,$cndb2) or die ("Error _comp [".$query_comp."]");
	...
	if($row_comp["status_fol"] == 'Y' && $row_comp["status_completed"] == 'Y')
	{
		...
		$strSQL_fol = "INSERT INTO tb_follow_customer(id_data,detail_fol,date_fol,date_appointment,login_emp,status_fol,status_completed,status_other) VALUES ('".$_POST["txt_iddata"]."','".$_POST["txt_detail"]."','$txt_date_fol','$txt_date_appointment','".strtoupper($_SESSION["4User"])."','$status_fol','$status_completed','Y')";
		mysql_select_db($db2,$cndb2);
		$objQuery_fol = mysql_query( $strSQL_fol ,$cndb2);
	}
	else
	{
		// Insert tb_follow_customer
		$strSQL_fol = "INSERT INTO tb_follow_customer(id_data,detail_fol,date_fol,date_appointment,login_emp,status_fol,status_completed) VALUES ('".$_POST["txt_iddata"]."','".$_POST["txt_detail"]."','$txt_date_fol','$txt_date_appointment','".strtoupper($_SESSION["4User"])."','$status_fol','$status_completed')";
		mysql_select_db($db2,$cndb2);
		$objQuery_fol = mysql_query( $strSQL_fol ,$cndb2);
	}
	...
	$query_user = "SELECT user_name,email From user where user_user='".strtoupper($_SESSION["4User"])."' ";	  
	mysql_select_db($db2,$cndb2);
