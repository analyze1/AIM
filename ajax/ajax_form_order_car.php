...
<div class='span12' style='width:100%;'>
		<div class="widget-box">
            <div class="widget-header widget-header-flat"> <h4>ข้อมูลสั่งซื้อรถยนต์</h4></div>
                <div class="widget-body">
                    <div class="widget-main">
                        <div class="row-fluid">
                            <div class="span12">
<form name='save_order' id='save_order'>
<table class="table">
<tr>
<td>
...
<a type='button' class='btn btn-info' onclick='save_order_car();'><i class='icon-save icon-white'></i> บันทึกสั่งซื้อรถ</a>
</div>
</span>
</td>
</tr> 
...
add_html+="<input type='text' name='car_price[]' class='span6' id='car_price"+row+"' value='0.00' onkeyup='cal_total("+row+");'>";
...
//add_html+="</div>";
add_html+="<div class='inline_add_one'>";
add_html+="<div class='inline_add_title'>ต้นทุนรถ</div>";
add_html+="<input type='text' name='car_cost[]' class='span6' id='car_cost"+row+"' value='0.00' readonly >";
add_html+="</div>";
add_html+="<div class='inline_add_one'>";
add_html+="<div class='inline_add_title'>ค่าBSC 2%</div>";
add_html+="<input type='text' name='car_bsc[]' class='span6' id='car_bsc"+row+"' value='0.00' readonly >";
add_html+="</div>";
add_html+="<div class='inline_add_one'>";
add_html+="<div class='inline_add_title'>จำนวนรถ</div>";
add_html+="<select name='unit_car[]' id='unit_car"+row+"' class='span6'>";
add_html+="<option value=''>--กรุณาเลือกจำนวนคัน--</option>";
<?php for($n=1;$n<=100;$n++){ ?>
add_html+="<option value='<?php echo $n; ?>'><?php echo $n." คัน"; ?></option>";
<?php } ?>
add_html+="</select>";
add_html+="</div>";
add_html+="<div class='inline_add_one'>";
add_html+="<div class='inline_add_title'>เงื่อนไขสั่งซื้อ</div>";
add_html+="<select name='condition_car[]' id='condition_car"+row+"' class='span6'>";
add_html+="<option value=''>--กรุณาเลือกเงือนไข--</option>";
add_html+="<option value='A สั่งซื้อล่วงหน้า (Advance)'>A สั่งซื้อล่วงหน้า (Advance)</option>";
add_html+="<option value='B สั่งซื้อลูกค้าจอง (Booking)'>B สั่งซื้อลูกค้าจอง (Booking)</option>";
add_html+="<option value='C สั่งซื้อเพื่อขาย(Stock)'>C สั่งซื้อเพื่อขาย(Stock)</option>";
add_html+="<option value='H สั่งซื้อเก็บตุน (Hold)'>H สั่งซื้อเก็บตุน (Hold)</option>";
add_html+="</select>";
add_html+="</div>";
...
$('#date_sale_show'+row).datepicker(
	...
	$('#car_bsc'+row).iMask({type : 'number'});
	$('#car_cost'+row).iMask({type : 'number'});
row++;
limit_row++;
}
...
add_row();
function save_order_car()
{
	if($("#login").val()=="")
		{
			alert("กรุณาเลือกสาขาผู้สั่งซื้อด้วยครับ");
			$("#login").focus();
			return false;
		}
	for(var i=0;i<document.getElementsByName("id_br_car[]").length;i++)
	{
		...
		if(document.getElementsByName("unit_car[]")[i].value=="")
		{
			alert("กรุณาเลือกจำนวนคันด้วยครับ");
			document.getElementsByName("unit_car[]")[i].focus();
			return false;
		}
		if(document.getElementsByName("condition_car[]")[i].value=="")
		{
			alert("กรุณาเลือกเงื่อนไขสั่งซื้อด้วยครับ");
			document.getElementsByName("condition_car[]")[i].focus();
			return false;
		}
	...
	var save_order = 
	{
		url:"ajax/ajax_save_order_car.php",
		dataType:"JSON",
		type:"POST",
		data:$("#save_order").serialize(),
		success:function(data)
		{
			if(data.status_save=='Y')
			{
				alert(data.alert);
				load_page('pages/select_order_car.php','ใบสั่งซื้อรถยนต์');
			}
			else
			{
				alert(data.alert);
...
{
	$.post("ajax/ajax_search_car_price.php",{id_mo_car_sub:$("#id_mo_car_sub"+row).val(),id_car_color:$("#id_car_color"+row).val()},function(data){$("#car_price"+row).val(data); if(data =='0' || data == '0.00' || data == ''){$("#car_price"+row).attr('readonly',false)}else{$("#car_price"+row).attr('readonly',true)}cal_total(row);});
}
...
function down_per_cal()
{
	...
	{
	var car_total = parseFloat($("#car_price"+row).val().replace(/,/g,''));
	var car_cost=(parseFloat(car_total) * parseFloat(95))/parseFloat(100);
	var car_bsc=(((parseFloat(car_total) * parseFloat(100))/parseFloat(107))* parseFloat(2))/ parseFloat(100);
	$("#car_cost"+row).val(addCommas(parseFloat(car_cost.toFixed(2))));
	$("#car_bsc"+row).val(addCommas(parseFloat(car_bsc.toFixed(2))));
	}
</script>