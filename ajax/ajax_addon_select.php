<?php
	...
	$date_addon = date("Y-m-d");

	$query_addon = "SELECT * from tb_addon Where 1=1 ";
	$query_addon .= "  AND end_date >= '" . $date_addon . "' ";
        if(!empty($data_id)){
	$query_addon .= "  AND code_addon = '" . $data_id . "' ";
        }
	$query_addon .= "  group by code_addon order by id";
	...
	{ 
		...
		$returnedArray[$i]['cost_insuran'] = $fetcharr[cost_insuran].' บาท';
		$i++;
	};
