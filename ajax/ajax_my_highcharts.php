...
Highcharts.chart('container_stock', {
    ...
    series: [{
		...
		<?php
		$n=0;
		$select_mo_sql="SELECT tb_stock_car.id_mo_car,tb_mo_car.name FROM tb_my_car
		INNER JOIN tb_stock_car ON (tb_my_car.id_stock = tb_stock_car.id_stock)
		INNER JOIN tb_mo_car ON (tb_stock_car.id_mo_car = tb_mo_car.id)
		".$sql." AND tb_my_car.id_stock != '0' GROUP BY tb_stock_car.id_mo_car";
		
		$select_mo_query=mysql_query($select_mo_sql,$cndb1);
		while($select_mo_array=mysql_fetch_array($select_mo_query))
		{ 
			$select_num_sql="SELECT tb_stock_car.id_stock FROM tb_my_car  
			INNER JOIN tb_stock_car ON (tb_my_car.id_stock = tb_stock_car.id_stock)
			".$sql." AND id_mo_car = '".$select_mo_array['id_mo_car']."'";
