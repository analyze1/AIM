...
$txtCC = $_POST['txtCC'];
$sendm = 0;
$Dbmy4ib = PDO_CONNECTION::fourinsure_mitsu();
$Dbfour = PDO_CONNECTION::fourinsure_insured();
$ExpAddress = new ConvertAddress($Dbmy4ib);
$query = "SELECT ";
$query .= "data.id,";
$query .= "data.p_act,";
$query .= "data.doc_type,";
...
if (!empty($carid_protect)) {
	...
	$pass_car_query = $Dbfour->query($pass_car_sql);
	$pass_car_array = $pass_car_query->fetch();
}
$protect_sql = "SELECT * FROM protect_09712 WHERE car_id='" . $carid_protect . "' AND DATE(exp_date) >= '" . $row['start_date'] . "'";
$protect_query = $Dbmy4ib->query($protect_sql);
...
$query1 = "SELECT
	tb_tumbon.`name` AS tumbon,
	tb_amphur.`name` AS amphur,
	tb_province.`name` AS province 
FROM
	req
	INNER JOIN tb_tumbon ON ( tb_tumbon.id = req.Cus_tumbon )
	INNER JOIN tb_amphur ON ( tb_amphur.id = req.Cus_amphur )
	INNER JOIN tb_province ON ( tb_province.id = req.Cus_province ) 
WHERE
req.id_data = '$row[id_data]'";

$objQuery1 = $Dbmy4ib->query($query1);

$row1 = $objQuery1->fetch();

$sqlMORE = "SELECT * FROM tb_acc";
$objQueryMORE = $Dbmy4ib->query($sqlMORE)->fetchAll(PDO::FETCH_ASSOC);
$costOb = array();
...
$objQueryMOREname = $Dbmy4ib->query($sqlMOREname)->fetchAll(PDO::FETCH_ASSOC);
$costObname = array();
...
#region ส่งเมล์แจ้งบริษัทประกันภัย
$pdf = new FPDF();
$pdf->AddPage();
...
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(45, 54, iconv('UTF-8', 'TIS-620', $protect_array['damage_out1']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
...
$pdf->Ln(6);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(45, 54, iconv('UTF-8', 'TIS-620', $protect_array['damage_notover']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
...
$pdf->Ln(6);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(45, 54, iconv('UTF-8', 'TIS-620', $protect_array['damage_cost']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
$pdf->Cell(13, 54, iconv('UTF-8', 'TIS-620', 'บาท/ครั้ง'), 0, 0, "L");
...
if ($row['car_cat_acc'] == '230') {
	...
	$frist = "-";
	$frist_1_1 = "3,000";
} else if ($row['car_id'] == "220" || $row['car_cat_acc'] == '220') {
	$frist = "-";
	$frist_1_1 = "-";
} else if ($row['car_id'] == '110' || $row['car_id'] == '320') {
	$frist = "-";
	$frist_1_1 = '1,500';
} else {
	$frist = "-";
...
$pdf->Ln(6);
$pdf->SetX(125);
$pdf->Cell(75, 54, iconv('UTF-8', 'TIS-620', '     ข) ผู้โดยสาร ' . $protect_array['people'] . ' คน'), 0, 0, "L");
$pdf->SetX(125);
$pdf->SetFont('angsa', 'B', 12);
...
$pdf->SetX(125);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(50, 54, iconv('UTF-8', 'TIS-620', $protect_array['pa5']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
...
$pdf->SetX(125);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(50, 54, iconv('UTF-8', 'TIS-620', $protect_array['pa6']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
...
$pdf->SetX(125);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(60, 54, iconv('UTF-8', 'TIS-620', $protect_array['pa3']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
$pdf->Cell(13, 54, iconv('UTF-8', 'TIS-620', 'บาท/คน'), 0, 0, "L");
...
$pdf->SetX(125);
$pdf->SetFont('angsa', 'B', 12);
$pdf->Cell(60, 54, iconv('UTF-8', 'TIS-620', $protect_array['pa4']), 0, 0, "R");
$pdf->SetFont('angsa', '', 12);
$pdf->Cell(2, 54, iconv('UTF-8', 'TIS-620', ''), 0, 0, "L");
$pdf->Cell(13, 54, iconv('UTF-8', 'TIS-620', 'บาท/ครั้ง'), 0, 0, "L");
$pdf->Ln(30);
...
$objQuery_UCostRenew = $Dbmy4ib->query($query_UCostRenew);
$row_UCostRenew = $objQuery_UCostRenew->fetch();
...
$objQuery_Passcar = $Dbmy4ib->query($query_Passcar);

$row_Passcar = $objQuery_Passcar->fetch();
...
if ($ShowReq == '' && $ShowReqOld == '') {
	...
	$pdf->SetFont('angsa', 'B', 12);
	$pdf->SetX(100);
	$pdf->Cell(60, 6, iconv('UTF-8', 'TIS-620', 'ประกันภัยตรง'), 0, 0, "L");
	$pdf->SetFont('angsa', '', 12);
	$pdf->SetX(120);
	$pdf->Cell(80, 6, iconv('UTF-8', 'TIS-620', 'ใบอนุญาตเลขที่'), 0, 0, "R");//ว00018/2551
	$pdf->Ln(6);
...
if ($row['com_data'] == 'VIB_C') {
	...
	if ($row['Email'] != "" && $row['Email']) { //Mail Four
...
$mail->AddAddress('admin@my4ib.com', "");
...
if ($row['equit'] == 'Y' || $row['Req_Status'] == 'Y') {
	...
	$new_SCT = explode("/", $status_company_time);
		...
		if ($row['equit'] == 'Y' and $row['Product'] == '0'  and $row['Product'] != '') {
			...
			for ($i = 0; $i < count($exitNum); $i++) {
				...
				if ($costOb['name'][$exitSplit[1]] <= 1) {
					if ($p < 6) {
						$equit0 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' 0 บาท ';
						$p++;
					} else if ($p < 12) {
						$equit1 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' 0 บาท ';
						$p++;
					} else if ($p < 18) {
						$equit2 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' 0 บาท ';
						$p++;
					}
					$TotalProduct += 0;
					...
					if ($p < 6) {
						$equit0 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' ' . number_format($costOb['name'][$exitSplit[1]], 0) . ' บาท ';
						$p++;
					} else if ($p < 12) {
						$equit1 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' ' . number_format($costOb['name'][$exitSplit[1]], 0) . ' บาท ';
						$p++;
					} else if ($p < 18) {
						$equit2 .= $costObname['name'][$row['cat_car']][$exitSplit[0]] . ' ' . number_format($costOb['name'][$exitSplit[1]], 0) . ' บาท ';
						$p++;
					}
	...
	if ($row['car_cat_acc_total'] != '0.00' && $row['car_cat_acc'] != '') {
		if ($row['car_cat_acc'] == '220') {
			if ($row['EditProduct'] == 'Y') {
				...
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (220 รถยนต์โดยสารใช้เพื่อการพาณิชย์) เพิ่มทุน ' . number_format($row['TotalProduct']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			} else {
				$detail_5 = 'อุปกรณ์ตกแต่งเพิ่มเติม : ' . $equit0 . ' ' . $equit1 . ' ' . $equit2;
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (220 รถยนต์โดยสารใช้เพื่อการพาณิชย์) เพิ่มทุน ' . number_format($row['price_total']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			}
		} else if ($row['car_cat_acc'] == '230') {
			if ($row['EditProduct'] == 'Y') {
				$detail_5 = 'อุปกรณ์ตกแต่งเพิ่มเติม : ' . $equit0 . ' ' . $equit1 . ' ' . $equit2;
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (230 รถยนต์โดยสารใช้รับจ้างสาธารณะ) เพิ่มทุน ' . number_format($row['TotalProduct']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			} else {
				$detail_5 = 'อุปกรณ์ตกแต่งเพิ่มเติม : ' . $equit0 . ' ' . $equit1 . ' ' . $equit2;
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (230 รถยนต์โดยสารใช้รับจ้างสาธารณะ) เพิ่มทุน ' . number_format($row['price_total']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			}
		} else if ($row['car_cat_acc'] == '120') {
			if ($row['EditProduct'] == 'Y') {
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (120  รถยน์นั่งใช้เพื่อการพาณิชย์ ไม่ใช่รับจ้างสาธารณะ) เพิ่มทุน' . number_format($row['TotalProduct']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			} else {
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (120  รถยน์นั่งใช้เพื่อการพาณิชย์ ไม่ใช่รับจ้างสาธารณะ) เพิ่มทุน' . number_format($row['price_total']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			}
		} else {
			if ($row['EditProduct'] == 'Y') {
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (' . $pass_car_array['pass_car_id'] . ' ' . $pass_car_array['name'] . ') เพิ่มทุน' . number_format($row['TotalProduct']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			} else {
				$detail_6 = 'สลักหลังเปลี่ยนประเภทรถ : (' . $pass_car_array['pass_car_id'] . ' ' . $pass_car_array['name'] . ') เพิ่มทุน' . number_format($row['price_total']) . ' บาท เพิ่มเบี้ย ' . number_format($row['car_cat_acc_total'], 2) . ' บาท ' . $car_cat_acc_text;
			}
		}
	} else {
		if ($row['EditProduct'] == 'Y') {
	...
	if (!empty($row['car_cat_acc'])) {
		$pdf->SetY(171);
		$pdf->SetX(10);
		$pdf->Cell(0, 7, iconv('UTF-8', 'TIS-620', $detail_6), 0, 0, "L");
	} else {
		$pdf->SetY(204);
		$pdf->SetX(33);
	...
	$mail2->AddAddress('admin@my4ib.com', "");
...
if ($row['Req_Status'] != 'Y') {
	if ($row['unit_edit'] == '0') {
		...
		$ACCOUNT = _UserSMS;
		$PASSWORD = _PassSMS; //"C699D09939D5BBF231EF67D073E3373C9C5A971CBEBCC45575CDB52CF9C3AEE4";
...
if ($sendm > 0) {
	...
	$objQuery = $Dbmy4ib->prepare($strSQL)->execute();
